...
public class BlockService {
    ...
    private Block getFristBlock() {
    	...
    	vo.setUser("测试First Block");
        return new Block(1, "0", System.currentTimeMillis(), vo, "aa212344fc10ea0a2cb885078fa9bc2354e55efc81be8f56b66e4a837157662e");
    }

    public Block generateNextBlock(DataVo blockData) {
        Block previousBlock = this.getLatestBlock();
        int nextIndex = previousBlock.getIndex() + 1;
        long nextTimestamp = System.currentTimeMillis();
        String nextHash = calculateHash(nextIndex, previousBlock.getHash(), nextTimestamp, blockData.toString());
    ...
    public List<Block> getBlockChain(int count) {
    	List<Block> list = new ArrayList<>();
    	int tempCount = count;
    	if(count <= 0) {
    		return list;
    	}
    	if(count > blockChain.size()) {
    		tempCount = blockChain.size();
    	}
    	for(int i = 0; i < tempCount; i++) {
    		list.add(blockChain.get(blockChain.size() - 1 - i));
    	}
        return list;
    }
}
